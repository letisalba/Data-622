...
```{r}
small_df_model <- rpart(Order.Priority ~ Region + Item.Type + Sales.Channel + Order.Date + Order.ID + Ship.Date + Units.Sold + Total.Revenue + Total.Cost + Total.Profit , method = "class", data = small_train)
...
```{r}
sum(diag(small_df_prediction)) / nrow(small_test)
```



```{r}
large_df_model <- rpart(Order.Priority ~ Region + Item.Type + Sales.Channel + Order.Date + Order.ID + Ship.Date + Units.Sold + Total.Revenue + Total.Cost + Total.Profit , method = "class", data = large_train)
...
```{r}
sum(diag(large_df_prediction)) / nrow(large_test)
```
