---
title: "Data 622 - Final Project: Homework 4"
...
#### [YouTube Video](https://youtu.be/wF2jTXtx3ec)
...
The code below wasn't working regardless of my efforts therefore excluded from my porject.

```{r, eval=FALSE}

#### Random Forest

#For the random forest model, I chose the second decision tree as it had a higher accuracy compared to the first model. Create the random forest model using the training data and then applying it to the validation data. Below are the results:
  
  # create some random number for reproduction 
set.seed(83)

# converting column 'Fetal_Health' to factor
fetal_df_cleaned2$Fetal_Health <- as.factor(fetal_df_cleaned2$Fetal_Health)

# create random forest model using the second training data
rf_model <- randomForest(Fetal_Health ~ AC + ALTV + ASTV + DP + MLTV + MSTV, train2)
rf_model

# prediction
rf_result <- predict(rf_model, newdata = valid2[, !colnames(valid2) %in% "Fetal_Health"])

# confusion matrix
confusionMatrix(rf_result, as.factor(valid2$Fetal_Health))
...
accuracy_rf <- confusionMatrix(rf_result, as.factor(valid2$Fetal_Health))$overall["Accuracy"]
accuracy_rf
```
